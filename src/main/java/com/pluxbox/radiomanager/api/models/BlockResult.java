...
public class BlockResult {
  ...
  private BlockRelationsItems items;
  ...
  * @throws IOException if the JSON Object is invalid with respect to BlockResult
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!BlockResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in BlockResult is not found in the empty JSON string", BlockResult.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!BlockResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BlockResult` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : BlockResult.openapiRequiredFields) {
      ...
      if (jsonObj.get("items") != null && !jsonObj.get("items").isJsonNull()) {
        BlockRelationsItems.validateJsonObject(jsonObj.getAsJsonObject("items"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!BlockResult.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'BlockResult' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<BlockResult> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(BlockResult.class));

       return (TypeAdapter<T>) new TypeAdapter<BlockResult>() {
           @Override
           public void write(JsonWriter out, BlockResult value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public BlockResult read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of BlockResult given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of BlockResult
  * @throws IOException if the JSON string is invalid with respect to BlockResult
  */
  public static BlockResult fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, BlockResult.class);
  }

 /**
  * Convert an instance of BlockResult to an JSON string
  *
  * @return JSON string
  */
